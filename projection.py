...
class Projection:
    ...
    def __init__(self,train_file,test_file):
        ...
        self.test_features = {}
        self.test_features['A'] = df.copy()
        self.test_features['G'] = df[df['position']=='G'].copy()
        self.test_features['F'] = df[df['position'].isin(['C','W'])].copy()
        self.test_features['D'] = df[df['position']=='D'].copy()

    def project_ols(self,f_feature_list,d_feature_list,g_feature_list):
        ...
        g_model = self.regression.ols('G',g_feature_list)
        #Predicts FP/60 using features in the test features and concats F, D, and G to have one series
        ...
        g_proj = pd.Series(g_model.predict(self.test_features['G'][g_feature_list]),self.test_features['G']['name'])
        pred = pd.concat([f_proj,d_proj,g_proj]).rename('proj_FP/60')
        
        return pred
    
    #Same as project OLS but with ridge regression
    def project_ridge(self,f_feature_list,d_feature_list,g_feature_list):
        f_model = self.regression.ridge('F',f_feature_list)
        d_model = self.regression.ridge('D',d_feature_list)
        g_model = self.regression.ridge('G',g_feature_list)
        f_proj = pd.Series(f_model.predict(self.test_features['F'][f_feature_list]),self.test_features['F']['name'])
        d_proj = pd.Series(d_model.predict(self.test_features['D'][d_feature_list]),self.test_features['D']['name'])
        g_proj = pd.Series(g_model.predict(self.test_features['G'][g_feature_list]),self.test_features['G']['name'])
        pred = pd.concat([f_proj,d_proj,g_proj]).rename('proj_FP/60')
    ...
    def export_projections(self,projections,file_name):
        df = self.test_features['A'].set_index(['date','name','position']).join(projections)
